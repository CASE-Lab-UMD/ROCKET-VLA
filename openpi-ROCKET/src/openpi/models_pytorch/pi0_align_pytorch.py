...
class PI0Pytorch(nn.Module):
    def __init__(self, config, extra_config):
        ...
        # Parse comma-separated layer indices into lists of ints
        self.vla_layers_align = [int(x) for x in extra_config.vla_layers_align.split(',')] if extra_config.vla_layers_align else [-1]
        self.vggt_layers_align = [int(x) for x in extra_config.vggt_layers_align.split(',')] if extra_config.vggt_layers_align else [-1]
    ...
    def forward(self, observation, actions, vggt, align_projectors, noise=None, time=None) -> Tensor:
        """Do a full training forward pass and compute the loss (batch_size x num_steps x num_motors)

        Args:
            align_projectors: nn.ModuleList of nn.ModuleList. Outer list is per alignment layer,
                inner list is ensemble of projectors for that layer.
        """
        ...
        # =================================== Alignment loss (multi-layer) ===================================

        # VGGT hidden states (computed once, shared across all alignment layers)
        ...
        # Precompute alignment mask (shared across all alignment layers)
        ...
        # non-rectangular image padding feature masks for alignment loss
        img_padding_mask_stacked = torch.stack(img_padding_mask, dim=1)
        target_size = img_padding_mask_stacked.shape[-1] // 14  # 224/14, where 14 is the patch size of Gemma encoder
        mask_downsampled = F.interpolate(
            img_padding_mask_stacked.float(),
        ...
        # Get the actual align_projectors module (unwrap DDP if needed)
        projectors_module = (
            align_projectors.module if isinstance(align_projectors, torch.nn.parallel.DistributedDataParallel)
            else align_projectors
        )

        # Multi-layer alignment: loop over each (vla_layer, vggt_layer) pair
        align_loss = 0
        align_loss_details = {}
        with torch.autocast("cuda", dtype=torch.bfloat16):
            for i, (vla_layer_idx, vggt_layer_idx) in enumerate(zip(self.vla_layers_align, self.vggt_layers_align)):
                # Get VLA hidden state for the current layer
                (prefix_hidden_i, _) = all_hidden_states[vla_layer_idx]
                vision_hidden_i = prefix_hidden_i[:, :img_len, :]

                # Get VGGT hidden state for the current layer
                agg_vggt_hidden_i = vggt_output["features"][vggt_layer_idx]
                ...
                pooled_vggt_hidden_i = custom_pooling(
                    vggt_hidden_i, (patch_h, patch_w), (H, W), vision_hidden_i, self.pooling_func, self.use_vggt_pe
                )

                # Compute alignment loss for this pair of layers (with ensemble)
                layer_projectors = projectors_module[i]
                current_layer_loss = 0
                for j, projector in enumerate(layer_projectors):
                    ens_loss = projector(vision_hidden_i, pooled_vggt_hidden_i, align_mask, layer_index=i)
                    align_loss_details[f"align_loss_layer_{i}_ens_{j}"] = ens_loss.item()
                ...
                align_loss_details[f"align_loss_layer_{i}_avg"] = current_layer_loss.item()
                align_loss += current_layer_loss

            # Average over alignment layers
            align_loss /= len(self.vla_layers_align)

        align_loss_details["align_loss"] = align_loss.item()
