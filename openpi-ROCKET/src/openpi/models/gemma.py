...
class Block(nn.Module):
    ...
    def __call__(self, xs, kv_cache, positions, attn_mask, adarms_cond, deterministic=True):  # noqa: FBT002
        ...
        # Pack hidden states for scan output (replace None with zeros placeholder for scan compatibility)
        hidden_out = tuple(x if x is not None else jnp.zeros(1, dtype=jnp.bfloat16) for x in xs)
        return xs, (kv_cache, hidden_out)
...
class Module(nn.Module):
    ...
    def __call__(
        ...
        *,
        ...
        embedded, (kv_cache, all_hidden_states) = self.layers(
        ...
        if output_hidden_states:
            return outputs, kv_cache, all_hidden_states
