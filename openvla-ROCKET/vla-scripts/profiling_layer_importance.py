...
class FinetuneConfig:
    ...
    measure_importance: bool = False                 # If True, measures layer importance and exits
    importance_save_file: Optional[str] = None       # Path to save importance scores
    num_calibration_batches: int = 20                # Number of batches to use for importance measurement
...
# def measure_layer_importance(vla, dataloader, num_batches, device_id, save_path, window_sizes=[1, 4, 8, 16, 24]):
def measure_layer_importance(vla, dataloader, num_batches, device_id, save_path, window_sizes=[1, 4, 8, 10, 16, 24]):
    print(f"Measuring layer importance over {num_batches} batches with windows {window_sizes}...")
    ...
    if hasattr(model, "base_model"):
        model = model.base_model.model # PeftModel -> BaseModel -> Original Model
    
    layers = None
    # Try common paths for OpenVLA / Prismatic
    possible_paths = [
    ...
    for path in possible_paths:
        ...
        if valid:
            layers = curr
            print(f"Found layers at path: {path}")
            break
            
    if layers is None:
        print("Could not find layers automatically. Printing module keys to help debug:")
        print(model)
        return
    ...
    # 2. Register hooks
    # We need to capture hidden states at each depth.
    # Depth 0: Input to Layer 0
    # Depth i+1: Output of Layer i (which is Input to Layer i+1)
    # Total depths: num_layers + 1
    
    # Dictionary to store accumulated stats: window_size -> start_idx -> {"sum": float, "count": int}
    import collections
    window_stats = {w: collections.defaultdict(lambda: {"sum": 0.0, "count": 0}) for w in window_sizes}
    
    # Storage for current batch hidden states
    ...
    def get_hook(layer_idx):
        def hook(module, input, output):
            # Input to the layer
            if layer_idx == 0:
                # input[0] is the hidden state tensor
                current_batch_states[0] = input[0].detach().cpu().float()
            ...
            current_batch_states[layer_idx + 1] = out_hidden.detach().cpu().float()
    ...
    vla.eval()
    with torch.no_grad():
        for i, batch in enumerate(tqdm.tqdm(dataloader, total=num_batches, desc="Computing Importance")):
            if i >= num_batches:
                break
            
            current_batch_states.clear()
            
            with torch.autocast("cuda", dtype=torch.bfloat16):
                 # Forward pass with minimal args required for Prismatic/OpenVLA
                 vla(
                    input_ids=batch["input_ids"].to(device_id),
                    attention_mask=batch["attention_mask"].to(device_id),
                    pixel_values=batch["pixel_values"].to(torch.bfloat16).to(device_id),
                    labels=batch["labels"].to(device_id) if "labels" in batch else None,
                    output_hidden_states=False,
                    # Pass proprio if present in batch
                    proprio=batch["proprio"].to(torch.bfloat16).to(device_id) if "proprio" in batch and batch["proprio"] is not None else None,
                )
            
            # Compute similarities for all windows
            # Ensure we have all states (sometimes partial batches might fail, check len)
            if len(current_batch_states) < num_layers + 1:
                continue

            for w in window_sizes:
                # Valid start indices: 0 to num_layers - w
                for start_idx in range(num_layers - w + 1):
                    # Compare Depth start_idx vs Depth start_idx + w
                    h_start = current_batch_states[start_idx]
                    h_end = current_batch_states[start_idx + w]
                    
                    # Compute Cosine Similarity
                    # Shape: (Batch, Seq, Dim) -> (Batch, Seq) -> Scalar
                    sim = F.cosine_similarity(h_start, h_end, dim=-1).mean().item()
                    
                    window_stats[w][start_idx]["sum"] += sim
                    window_stats[w][start_idx]["count"] += 1
                
    # 4. Cleanup
    for h in hooks:
        h.remove()
        
    # 5. Aggregate results (Distributed Reduction)
    print(f"[Device {device_id}] Aggregating results from all GPUs...")
    for w in window_sizes:
        for start_idx in range(num_layers - w + 1):
            stats = window_stats[w][start_idx]
            # Create a tensor [sum, count] for reduction
            # Use float64 for precision accumulation
            t = torch.tensor([stats["sum"], stats["count"]], dtype=torch.float64, device=device_id)
            
            # Sum up stats across all GPUs
            if dist.is_initialized():
                dist.all_reduce(t, op=dist.ReduceOp.SUM)
            
            # Update local stats with the aggregated values
            stats["sum"] = t[0].item()
            stats["count"] = int(t[1].item())

    # Only Main Process (Rank 0) prints and saves results
    if not dist.is_initialized() or dist.get_rank() == 0:
        print("\n" + "="*80)
        print("LAYER SIMILARITY ANALYSIS (Sliding Window) - GLOBAL RESULTS")
        print("="*80)
        ...
        for w in sorted(window_sizes):
            print(f"\n[Window Size: {w} layers]")
            print(f"Description: Similarity between Input of Layer i and Output of Layer i+{w-1}")
            print("-" * 60)
            
            results = []
            # start_idx is 0-based index of the first layer in the window
            for start_idx in range(num_layers - w + 1):
                stats = window_stats[w][start_idx]
                if stats["count"] > 0:
                    avg_sim = stats["sum"] / stats["count"]
                    results.append((start_idx, avg_sim))
            
            # Save results for this window size (sorted by index)
            all_results_to_save[w] = {r[0]: r[1] for r in sorted(results, key=lambda x: x[0])}

            # Sort by similarity (ascending) for printing
            results.sort(key=lambda x: x[1])
            
            print(f"Total Windows: {len(results)}")
            print("\nAll Windows (Sorted by Similarity Low->High):")
            print("Start Layer (1-based) | Similarity")
            for idx, sim in results:
                print(f"Layer {idx+1:<16} | {sim:.4f}")

            # Construct lists for copy-pasting
            sorted_indices = [x[0] + 1 for x in results] # 1-based offset
            
            print("\n[Summary for Copying]")
            print(f"Window {w} - Sorted Layer Indices (Low Sim -> High Sim):")
            print(",".join(map(str, sorted_indices)))

        if save_path:
            torch.save(all_results_to_save, save_path)
            print(f"\nSaved all window similarities to {save_path}")
    
    # Barrier to ensure all processes wait for main process to finish printing/saving
    if dist.is_initialized():
        dist.barrier()
...
def finetune(cfg: FinetuneConfig) -> None:
    ...
    if cfg.measure_importance:
        measure_layer_importance(vla, dataloader, cfg.num_calibration_batches, device_id, cfg.importance_save_file)
        return
