...
from prismatic.models.projectors_rocket import NoisyActionProjector, ProprioProjector
...
def _apply_film_to_vla(vla: torch.nn.Module, cfg: Any) -> torch.nn.Module:
    ...
    vla = vla.model
    vla.vision_backbone = vla.vision_backbone.to(torch.bfloat16).to(cfg.device)
...
def get_noisy_action_projector(cfg: Any, llm_dim: int) -> NoisyActionProjector:
    ...
    noisy_action_projector = noisy_action_projector.to(torch.bfloat16).to(cfg.device)
